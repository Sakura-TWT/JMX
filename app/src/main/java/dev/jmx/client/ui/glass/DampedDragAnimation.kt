...
class DampedDragAnimation(
    ...
    val onDragStarted: DampedDragAnimation.(Offset) -> Unit,
    ...
    val modifier: Modifier = Modifier.pointerInput(requireLongPress) {
        if (requireLongPress) {
    ...
    fun animateToValue(value: Float) {
        valueJob?.cancel()
        selectionJob?.cancel()
        selectionJob = animationScope.launch {
            mutatorMutex.mutate {
                press()
                val targetValue = value.coerceIn(valueRange)
    ...
    fun animateSelectionToValue(value: Float) {
        val targetValue = value.coerceIn(valueRange)
        valueJob?.cancel()
        selectionJob?.cancel()
        selectionJob = animationScope.launch {
